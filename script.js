document.addEventListener('DOMContentLoaded', () => {
    ...
    createPlaylistBtn.addEventListener('click', () => {
        ...
        if (playlistName && playlistName.trim() !== "") {
            ...
            if (!userPlaylists[trimmedName]) {
                ...
                // Replace alert with custom modal or message
                // alert(`Playlist "${trimmedName}" creada!`);
                displayMessage(`Playlist "${trimmedName}" creada!`);
            } else {
                // Replace alert with custom modal or message
                // alert(`La playlist "${trimmedName}" ya existe.`);
                displayMessage(`La playlist "${trimmedName}" ya existe.`);
            }
        }
    });
    ...
    function removeSongFromPlaylist(playlistName, songId) {
        // Replace confirm with custom modal or message
        // if (confirm(`¿Estás seguro de que quieres eliminar esta canción de la playlist "${playlistName}"?`)) {
        displayConfirmation(`¿Estás seguro de que quieres eliminar esta canción de la playlist "${playlistName}"?`, () => {
    ...
    function deletePlaylist(playlistName) {
        ...
        displayConfirmation(`¿Estás seguro de que quieres eliminar la playlist "${playlistName}"? Esta acción es irreversible.`, () => {
            ...
            displayMessage(`Playlist "${playlistName}" eliminada.`);
        });
    }
        ...
        if (playlistChoice && playlistChoice.trim() !== "") {
            ...
            if (userPlaylists[trimmedChoice]) {
                ...
                if (!isAlreadyInPlaylist) {
                    ...
                    displayMessage(`"${song.title}" añadida a la playlist "${trimmedChoice}".`);
                } else {
                    // Replace alert with custom modal or message
                    // alert(`"${song.title}" ya está en la playlist "${trimmedChoice}".`);
                    displayMessage(`"${song.title}" ya está en la playlist "${trimmedChoice}".`);
                }
            } else {
                // Replace alert with custom modal or message
                // alert(`La playlist "${trimmedChoice}" no existe. Crea una primero.`);
                displayMessage(`La playlist "${trimmedChoice}" no existe. Crea una primero.`);
            }
        }
    }
    ...
    function addToQueue(song) {
        ...
        displayMessage(`"${song.title}" añadida a la fila.`);
    }
    ...
    function setupMouseIdleDetection() {
    ...
    // --- Custom Alert/Confirmation (replaces native alert/confirm) ---
    // Create a simple modal for alerts/confirmations
    function createModal() {
        const modal = document.createElement('div');
        modal.id = 'custom-modal';
        modal.classList.add('custom-modal'); // Add a class for styling
        modal.innerHTML = `
    ...
    function displayConfirmation(message, onConfirm) {
        modalMessage.textContent = message;
        modalCancelBtn.style.display = 'inline-block'; // Show cancel button for confirmations
    ...
    // Add basic styles for the modal (can be moved to style.css)
